...
namespace RayTracingEngine
{
	...
	public:
		...
		bool Import3DS(CScene* pModel, char *strFileName);
	...
	private:
		...
		void ProcessNextChunk(CScene *pModel, tChunk *);

		// This reads the object chunks
		void ProcessNextObjectChunk(CScene *pModel, CModel *pObject, tChunk *);

		// This reads the material chunks
		void ProcessNextMaterialChunk(CScene *pModel, tChunk *);

		// This reads the RGB value for the object's color
		void ReadColorChunk(CMaterial *pMaterial, tChunk *pChunk);

		// This reads the objects vertices
		void ReadVertices(CModel *pObject, tChunk *);

		// This reads the objects face information
		void ReadVertexIndices(CModel *pObject, tChunk *);

		// This reads the texture coodinates of the object
		void ReadUVCoordinates(CModel *pObject, tChunk *);

		// This reads in the material name assigned to the object and sets the materialID
		void ReadObjectMaterial(CScene *pModel, CModel *pObject, tChunk *pPreviousChunk);

		// This computes the vertex normals for the object (used for lighting)
		void ComputeNormals(CScene *pModel);
